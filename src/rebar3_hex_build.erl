%% @doc The build provider is responsible for creating a tarball.
%% @end
...
init(State) ->
    Provider = providers:create([{name, ?PROVIDER},
                                 ...
                                 {short_desc, "Build tarball of the package"},
                                 {desc, ""},
...
format_error({validation_errors, Errors, Message}) ->
    ...
    io_lib:format("Failed to build package: ~ts~n\t~ts", [Message, ErrorString]);
format_error({build_failed, Message}) ->
    io_lib:format("Failed to build package: ~ts", [Message]);
...
build(App, HexConfig, State) ->
    ...
    case is_valid_app({App, Name, Version, AppDetails}) of
        ok ->
            build(AppDir, Name, ResolvedVersion, TopLevel,
                    Excluded, AppDetails, HexConfig, State);
        {error, Errors} ->
            ?PRV_ERROR(Errors)
    end.

build(AppDir, Name, Version, Deps, [], AppDetails, HexConfig, State) ->
    ...
    ec_talk:say("Building ~ts ~ts to ~ts", [PkgName, Version, maps:get(name, HexConfig)]),
    ...
    case build_package(Metadata, PackageFiles, HexConfig, Name, Version) of
        ok ->
            rebar_api:info("Successfully created ~s ~s", [Name, Version]),
            {ok, State};
        Error={error, _} ->
            Error
    end;
build(_AppDir, _Name, _Version, _Deps, Excluded, _AppDetails, _, _) ->
...
build_package(Metadata, PackageFiles, HexConfig, Name, Version) ->
    ...
    rebar_api:info("Checksum: ~s", [Ch16]),
    rebar_api:info("Filename: ~s", [Filename]),
    file:write_file(Filename, Tarball).
...
validate_app_details(AppDetails) ->
    case proplists:is_defined(contributors, AppDetails) of
        true ->
            {error, {rebar3_hex_build, has_contributors}};
        false ->
            ok
    end.
